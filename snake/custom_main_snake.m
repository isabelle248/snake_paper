function dist = custom_main_snake(amp, freq, wavelen, phase, loco_type, env_type)
    ...
    % % ADDED
    % beg_current_pos_x = zeros(Nsteps,1);
    % beg_current_pos_y = zeros(Nsteps,1);
    % beg_current_pos_z = zeros(Nsteps,1);
    % beg_log_node = 1;
    % beg_current_pos_x(1) = softRobot.q0(3*beg_log_node-2);
    % beg_current_pos_y(1) = softRobot.q0(3*beg_log_node-1);
    % beg_current_pos_z(1) = softRobot.q0(3*beg_log_node);
    % 
    % mid_current_pos_x = zeros(Nsteps,1);
    % mid_current_pos_y = zeros(Nsteps,1);
    % mid_current_pos_z = zeros(Nsteps,1);
    % mid_log_node = 11;
    % mid_current_pos_x(1) = softRobot.q0(3*mid_log_node-2);
    % mid_current_pos_y(1) = softRobot.q0(3*mid_log_node-1);
    % mid_current_pos_z(1) = softRobot.q0(3*mid_log_node);
    % %
    ...
    for timeStep = 1:Nsteps
        ...
        % % ADDED
        % beg_current_pos_x(timeStep) = softRobot.q0(3*beg_log_node-2);
        % beg_current_pos_y(timeStep) = softRobot.q0(3*beg_log_node-1);
        % beg_current_pos_z(timeStep) = softRobot.q0(3*beg_log_node);
        % 
        % mid_current_pos_x(timeStep) = softRobot.q0(3*mid_log_node-2);
        % mid_current_pos_y(timeStep) = softRobot.q0(3*mid_log_node-1);
        % mid_current_pos_z(timeStep) = softRobot.q0(3*mid_log_node);
        % %
    ...
    % 
    % % ADDED
    % writematrix(beg_current_pos_x, filename, Sheet=1,Range='E1');
    % writematrix(beg_current_pos_y, filename, Sheet=1,Range='F1');
    % writematrix(beg_current_pos_z, filename, Sheet=1,Range='G1');
    % 
    % writematrix(mid_current_pos_x, filename, Sheet=1,Range='H1');
    % writematrix(mid_current_pos_y, filename, Sheet=1,Range='I1');
    % writematrix(mid_current_pos_z, filename, Sheet=1,Range='J1');
    % %
